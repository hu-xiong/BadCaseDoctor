    ...
    if _GrepCfg.GREP_ES_WARMUP and _GrepCfg.GREP_VECTOR_ENABLED:
        ...
        def _grep_es_warmup_worker():
            try:
                ...
                _es_to = float(getattr(_GrepCfg, "GREP_ES_SEARCH_TIMEOUT_S", 10) or 10)
                _store.es.info(request_timeout=_es_to)
...
def _probe_tcp_bind(host: str, port: int):
    """启动前探测 TCP 能否绑定；成功返回 None，失败返回错误字符串。"""
    import socket
