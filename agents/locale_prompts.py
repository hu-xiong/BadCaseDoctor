...
def modify_tool_progress(key: str, locale: Optional[str], **kw: Any) -> str:
    if is_english_locale(locale):
        m = {
            ...
            "priority_norm": "Priority normalized: {orig} -> {norm}",
            "case_category_norm": "Problem category normalized: {orig} -> {norm}",
        ...
        m = {
            ...
            "priority_norm": "优先级归一化：{orig} -> {norm}",
            "case_category_norm": "问题分类归一化：{orig} -> {norm}",
