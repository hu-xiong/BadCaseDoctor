...
本仓库已能演示主链路，但**距离「开箱即用、长期无人值守提测」还有一点距离**。分享或试用时请按下面预期：


| 能力                                     | 状态      | 说明                                   |
| -------------------------------------- | ------- | ------------------------------------ |
| Agent **自动提** Bug / BadCase / TestCase | **较完善** | 对话内起草、落库草稿可用                         |
| **Diff Review**（采纳 / 拒绝 / 合并最新 Diff）   | **较完善** | 人审闭环已打通，可交给 Cursor 等继续修              |
| 项目 / 权限 / 额度 / 本机 Go 代理 / 终端           | **可用**  | 个人开发者联调路径成立                          |
| Agent **自动探测界面找 Bug**                  | **需完善** | CDP/Explore 已有骨架，稳定巡检、误报控制、复杂站适配仍要加强 |


一句话：**「提 Case + Diff 采纳」侧更成熟；「自主探测发现 Bug」侧还在补齐。** 试用建议先走对话提 Case / Diff，探测能力当预览能力看待。
...
个人开发者场景里，本机代理往往是 **首次打开客户端时现下** 的。若用 Node/Python 运行时或重型 sidecar，动辄几十上百 MB，等待感很强。

选用 **Go 静态编译单文件** 后，分发体积大约 **6MB 量级**（见 `client_binaries/`），大大缩短下载与冷启动等待，也更适合「装上就能测内网」的轻量工具定位——不附带解释器、不拖一整套依赖树。

### 边缘计算：算力可以沉到终端 Agent

云端保留 LLM 编排与业务状态即可；**重计算、本地脚本、构建/测试、页面侧采集与预处理**，后续都可以沉到本机终端 Agent（经 Go 代理跑 bash / 工具链）上做 **边缘计算**：

- 用户机器就近执行，少占云端 CPU / GPU / 长连接沙箱  
- 内网数据不必整包上传，只回传摘要与结论  
- **直接压低服务器费用**，个人开发者与小团队也能养得起这条链路

也就是说：Go 代理不仅是「够得到内网」的管道，还是把 **Everything is Bash** 扩成边缘算力面的底座。
